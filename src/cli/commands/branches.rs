...
impl BranchesOpt {
    ...
    pub fn list_branches(git: &git2::Repository) -> Result<Vec<NamedBranchInfo>> {
        let mut branches = git
            ...
            .map(|(branch, _ty)| {
                ...
                return NamedBranchInfo { name, commit_hash, datetime };
            })
    ...
    pub fn display_branch_list(git: &git2::Repository) -> Result<()> {
        ...
        let max_name_length = branches.iter().map(|bi| bi.name.len()).max().unwrap_or_default();
        let max_datetime_length = branches.iter().map(|bi| bi.datetime_string().len()).max().unwrap_or_default();
        ...
        if self.new_branch_name.is_none() && self.delete_branch_name.is_none() {
            ...
            match git.branch(&branch_name, &commit, self.force) {
                ...
                Err(error) => match error.code() {
                    ...
                    _code => {
                        eprintln!("failed to create branch {branch_name:#?}: {error}");
                    }
                },
            }
...
impl NamedBranchInfo {
    ...
    pub fn get_hash_color_ansi_sequence(&self, reset: bool, algo: Contrast) -> Result<String> {
        let hash_color = self.get_hash_color_rgb()?;
        let hash_color_fg = hash_color.to_ansi(Layer::BG, true);
        let hash_color_bg = algo.apply(hash_color, Layer::BG)?.to_ansi(Layer::FG, true);
        let reset = if reset { "\x1b[0m" } else { "" };
        let ansi_sequence = format!("{reset}{hash_color_fg}{hash_color_bg}");
        Ok(ansi_sequence)
    }

    pub fn get_name_color_ansi_sequence(&self, reset: bool, algo: Contrast) -> Result<String> {
        let hash_color = self.get_hash_color_rgb()?;
        let name_color_fg = hash_color.to_ansi(Layer::FG, true);
        let name_color_bg = algo.apply(hash_color, Layer::BG)?.to_ansi(Layer::BG, true);
        let reset = if reset { "\x1b[0m" } else { "" };
    ...
    pub fn to_term_line(&self, colorize: bool, name_width: usize, date_width: usize) -> Result<String> {
        ...
        let hash = if colorize {
            let ansi_color = self.get_hash_color_ansi_sequence(true, Contrast::Read)?;
            format!("{ansi_color}{hash}\x1b[0m")
        } else {
            hash
        };
        let name = if colorize {
            let ansi_color = self.get_name_color_ansi_sequence(true, Contrast::Read)?;
...
impl std::fmt::Display for NamedBranchInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{line}", line = self.to_term_line(false, self.name.len(), self.datetime_string().len()).expect("branch to string"))
    }
}
