...
pub struct GitIgnoreAddOpt {
    ...
    #[arg(short, long, help = "write changes to .gitignore instead of printing to stdout")]
    pub write: bool,
...
impl ArgsDispatcher<Error> for GitIgnoreAddOpt {
    fn dispatch(&self) -> Result<()> {
        ...
        let mut lines: BTreeSet<String> = if git_ignore_path.is_file() {
            git_ignore_path.read_lines()?.into_iter().collect::<BTreeSet<String>>()
        ...
        let now = Local::now();
        let banner = format!("# added by git-bandolier via {argv} on {now}:", argv = iocore::env::args().join(" "));
...
/// succeeded match
pub fn match_pattern_with_strategies(pattern_str: &str, haystack: &str) -> Option<(MatchStrategy, String)> {
    ...
    fn dispatch(&self) -> Result<()> {
        ...
        for pat_raw in self.patterns() {
            ...
            let mut indexes_to_delete = lines
                .iter()
                .map(|haystack| match_pattern_with_strategies(pat_raw.as_str(), haystack.as_str()))
                .enumerate()
