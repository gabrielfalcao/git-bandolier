    ...
    fn repo_path_from_repo(&self, repo: &Repository) -> Result<Path>
    {
        ...
        match git_bare_path.parent()
        {
            ...
            None => Err(Error::IOError(format!(
                "cannot obtain parent of path {git_bare_path}"
            ))),
        }
    }
    ...
    fn git_ignore_path(&self) -> Result<Path>
    {
        ...
        if ignore_path.exists() && !ignore_path.is_file()
        {
            return Err(Error::IOError(format!(
                "gitignore exists but is not a file: {ignore_path}"
            )));
        }
        Ok(ignore_path)
    }
    fn git_status_list_untracked_paths(
        &self,
        recurse_untracked: Option<bool>,
    ) -> Result<Vec<Path>>
    {
        let repo = self.git_repo()?;
        ...
        {
            let path_string = SString::new(item.path_bytes())
                .safe()
